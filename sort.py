    ...
    for root, dirs, files in os.walk(folder_path):
        for filename in files:
            extension = os.path.splitext(
                filename)[1][1:].upper()  # Розширення файлу
            ...
            # Категорії файлу за розширенням
            if extension in image_extensions:
                ...
                # Видалення розширення з імені архіву
                archive_name = os.path.splitext(normalized_filename)[0]
                # Додання підпапки з іменем архіву
                destination_folder = os.path.join(
                    destination_folder, archive_name)
                os.makedirs(os.path.join(
                    root, destination_folder), exist_ok=True)
                extract_archive(source_file_path, os.path.join(
            ...
            destination_file_path = os.path.join(
                destination_folder_path, new_filename)
